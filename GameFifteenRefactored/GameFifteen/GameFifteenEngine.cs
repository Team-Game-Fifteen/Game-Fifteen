namespace GameFifteen
{
    ...
    class GameFifteenEngine
    {
        ...
        private static void UpgradeTopScore()
        {
            ...
            HighScore[] topScoresPairs = UpgradeTopScorePairs(topScores);

            IOrderedEnumerable<HighScore> sortedScores =
            topScoresPairs.OrderBy(x => x.Score).ThenBy(x => x.Name);

            UpgradeTopScoreInFile(sortedScores);
        }

        private static void UpgradeTopScoreInFile(IOrderedEnumerable<HighScore> sortedScores)
        {
            StreamWriter topWriter = new StreamWriter(TopScoresFileName);
            using (topWriter)
            {
                int position = 1;
                foreach (HighScore pair in sortedScores)
                {
        ...
        private static HighScore[] UpgradeTopScorePairs(string[] topScores)
        {
            int startIndex = 0;
            ...
            HighScore[] topScoresPairs = new HighScore[arraySize];
            for (int topScoresPairsIndex = 0; topScoresPairsIndex < arraySize; topScoresPairsIndex++)
            {
                ...
                topScoresPairs[topScoresPairsIndex] = new HighScore(name, scoreInt);
            }
